                ...
                @foreach ($modules as $module)
                    @if ($module->filiere_id == $filiere->id)
                        ...
                        <div id="affecterpopupfor{{ $module->id }}" class="overlay">
                            <div class="popup bg-white rounded p-4 shadow"
                                ...
                                <form action="{{ url('chef/modules_vacantes/affecter/' . $module->id) }}" method="post">
                                    @csrf
                                    <h5 class="text-primary fw-bold mb-3">Affecter le module <span
                                            id="affectermoduleName{{ $module->id }}">{{ $module->name }}</span> a un
                                        professeur</h5>


                                    <!-- Select Professor -->
                                    <div class="mb-4 mt-5 d-flex flex-column align-items-center">
                                        <label style="color:#515151; font-weight: 700; width:100%;" for="professor"
                                            class="form-label fw-bold text-start pb-1">Professeurs</label>
                                        <select class="form-select py-2 rounded" id="professor" name="prof_id">
                                            <option value="">-- Sélectionner un professeur --</option>
                                            @foreach ($professors as $professor)
                                                <option value="{{ $professor->id }}">{{ $professor->lastname }}
                                                    {{ $professor->firstname }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="text-end mt-4">
                                        ...
                                        <button type="button" class="btn btn-secondary btn-sm"
                                            onclick="closePopupaffecter({{ $module->id }})">Annuler</button>
                        ...
                        <div id="popupfor{{ $module->id }}" class="overlay">
                            <div class="popup bg-white rounded p-4 shadow"
                                style="max-width: 600px; width: 90%; margin: auto; position: relative;">

                                <!-- Title -->
                                <h5 class="text-primary fw-bold mb-3">Détails du module</h5>

                                <!-- Module Name -->
                                <p class="text-muted mb-4"><i class="bi bi-book me-2"></i><span
                                        id="moduleName{{ $module->id }}">{{ $module->name }}</span></p>
                                ...
                                <div class="row gy-3">
                                    ...
                                    @if ($module->responsable)
                                        <div class="col-12">
                                            <strong>Responsable:</strong> {{ $module->responsable->firstname }}
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                            <div class="module-container  shadow-sm bg-white h-100 d-flex flex-column align-items-center p-3 text-center position-relative"
                                style="border-radius: 10px;">



                                <!-- Module Code Badge -->
                                <span class="position-absolute top-0 end-0 text-white bg-primary px-3 py-1 "
                                    style="font-size: 0.8rem; font-weight: 600; border-bottom-left-radius:10px ;border-top-right-radius:10px;">
                                    {{ $module->code }}
                                </span>
                                ...
                                <!-- Professor -->
                                @if ($module->professor)
                                    <p class=" fw-semibold mb-3" style="vertical-align: middle ; color: #4723d9;"><i
                                            class="bi bi-person-circle me-1 "></i>{{ $module->professor->firstname }}
                                        {{ $module->professor->lastname }}</p>
                                @else
                                    <p class="mb-3">
                                ...
                                <div class="d-flex justify-content-center gap-3 flex-wrap">
                                    <div class="text-center">
                                        ...
                                        <span
                                            class="badge bg-success rounded-pill px-3">{{ $module->cm_hours }}h</span>
                                    </div>
                                    <div class="text-center">
                                        <span class="text-muted small d-block">TP</span>
                                        <span
                                            class="badge bg-success rounded-pill px-3">{{ $module->tp_hours }}h</span>
                                    </div>
                                    <div class="text-center">
                                        <span class="text-muted small d-block">TD</span>
                                        <span
                                            class="badge bg-success rounded-pill px-3">{{ $module->td_hours }}h</span>
                                    </div>
                                </div>
                                ...
                                <div class="buttons-container mt-4 d-flex gap-2"><button class="btn"
                                        style="border: 1px solid #4723d9; color: #4723d9;"
                                        onclick="showPopupaffecter({{ $module->id }}, '{{ $module->name }}')">Affecter</button><button
                                        class="btn btn-success bg-white"
                                        style=" color:#18a16f; border: 1px solid #18a16f;"
                                        onclick="showPopup({{ $module->id }}, '{{ $module->name }}')">Voire
                                        plus</button></div>
                            </div>
                        </div>
                        @php
