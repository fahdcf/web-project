            ...
            foreach ($requests as $request) {
                if ($request->module) {
                    ...
                    if (in_array('td', $groupTypes) && !empty($tdGroups)) {
                        $tdHoursPerGroup =
                            ($request->module->td_hours ?? 0) / ($request->module->td_groups_available ?: 1);
                    ...
                    if (in_array('tp', $groupTypes) && !empty($tpGroups)) {
                        $tpHoursPerGroup =
                            ($request->module->tp_hours ?? 0) / ($request->module->tp_groups_available ?: 1);
            ...
            <div class="card-body p-0">
                @if ($requests->isEmpty())
                    <div class="alert alert-info m-4">
                        <i class="fas fa-info-circle me-2" aria-hidden="true"></i> Vous n'avez aucune demande pour le
                        moment.
                    </div>
                @else
                                    ...
                                    @php
                                        ...
                                        if (in_array('td', $groupTypes) && !empty($tdGroups)) {
                                            $tdHoursPerGroup =
                                                ...
                                                ($request->module->td_groups_available ?: 1);
                                            $requestHours += $tdHoursPerGroup * count($tdGroups);
                                        }
                                        ...
                                        if (in_array('tp', $groupTypes) && !empty($tpGroups)) {
                                            $tpHoursPerGroup =
                                                ...
                                                ($request->module->tp_groups_available ?: 1);
                                            $requestHours += $tpHoursPerGroup * count($tpGroups);
                                        }
                                        ...
                                        $isEntireModule =
                                            in_array('cm', $groupTypes) &&
                                            $request->module->cm_groups_available > 0 &&
                                            (in_array('td', $groupTypes) &&
                                                count($tdGroups) == $request->module->td_groups_available) &&
                                            (in_array('tp', $groupTypes) &&
                                                count($tpGroups) == $request->module->tp_groups_available);
                                    @endphp
                                    <tr>
                                        <td>
                                            ...
                                            <div class="small text-muted">
                                                <span
                                                    class="badge bg-info text-white">S{{ $request->module->semester }}</span>
                                                {{ $request->module->code }}
                                                ...
                                                <div class="d-flex flex-wrap gap-1">
                                                    ...
                                                    @if (in_array('td', $groupTypes) && !empty($tdGroups))
                                                        @foreach ($tdGroups as $group)
                                                            <span
                                                                class="badge bg-warning text-dark">TD{{ $group }}</span>
                                                        @endforeach
                                                    @endif
                                            ...
                                            <div class="small">
                                                ...
                                                @if (in_array('td', $groupTypes) && !empty($tdGroups))
                                                    <div>
                                                        ...
                                                        TD:
                                                        {{ number_format((($request->module->td_hours ?? 0) / ($request->module->td_groups_available ?: 1)) * count($tdGroups), 1) }}h
                                                        <span class="text-muted">({{ count($tdGroups) }}
                                                            groupe(s))</span>
                                                    </div>
                                                @endif
                                                ...
                                                @if (in_array('tp', $groupTypes) && !empty($tpGroups))
                                                    <div>
                                                        ...
                                                        TP:
                                                        {{ number_format((($request->module->tp_hours ?? 0) / ($request->module->tp_groups_available ?: 1)) * count($tpGroups), 1) }}h
                                                        <span class="text-muted">({{ count($tpGroups) }}
                                                            groupe(s))</span>
                                                    </div>
                                                @endif
